...
# 창 제목 설정
pygame.display.set_caption("화면 분할 및 캐릭터 이동")
...
# 버튼 리스트 초기화
buttons = []
button_texts = ["선택지 1", "선택지 2", "선택지 3", "선택지 4", "선택지 5", "이미지 퍼즐 게임 시작"]  # "이미지 퍼즐 게임 시작" 버튼 추가
button_width = 200
button_height = 40
button_gap = 20

# 버튼의 시작 위치를 화면 중앙으로 설정
button_start_x = (screen_width - button_width) // 2  # 화면 중앙 x
button_start_y = 100  # 상단 텍스트 아래 y 위치
...
# 선택지 버튼 액션 정의
def select_option_1():
    # 원하는 순서대로 이동 명령을 큐에 추가
    move_camera("left", 430)  # 카메라 좌측으로 430 이동
    move_camera("up", 270)    # 카메라 상단으로 270 이동
    movement_queue.append(("start_game", "avoid_star"))  # 이동 후 게임 시작

def select_option_2():
    # 원하는 순서대로 이동 명령을 큐에 추가
    move_camera("left", 210)  # 카메라 좌측으로 210 이동
    move_camera("up", 270)    # 카메라 상단으로 270 이동
    movement_queue.append(("start_game", "image_puzzle"))  # 이동 후 게임 시작
...
# 버튼에 따른 동작 매핑 수정
button_actions = {
    "선택지 1": select_option_1,  # "선택지 1" 클릭 시 카메라 이동 후 게임 시작
    "선택지 2": select_option_2,  # "선택지 2" 클릭 시 카메라 이동 후 게임 시작
    "선택지 3": select_option_3,  # "선택지 3" 클릭 시 특정 이동
    "선택지 4": select_option_4,  # "선택지 4" 클릭 시 특정 이동
    "선택지 5": select_option_5,  # "선택지 5" 클릭 시 카메라 리셋
    "이미지 퍼즐 게임 시작": start_image_puzzle_game,  # 기존 기능 유지
} 
